...
class Element:
    ...
    def get_child_element(self, child_locator: 'Element', raise_exception=True) -> 'Element':
        ...
        """
        try:
            child_element = self.find().element.find_element(child_locator.locator)
            if child_element:
                child_locator.element = child_element
    ...
    def get_child_elements(self, child_locator: 'Element', raise_exception=True) -> 'Element':
        """Get child elements from parent element
        Arguments:
            child_locator: Element instance initialized with child locator
            raise_exception (bool): True or False
    ...
    def __getitem__(self, index: int):
        ...
        if 0 <= index < len(self.elements):
            ...
            raise NotFoundError(f"The index: {index} you are tring to access does not exists in list of len: {len(self.elements)}")
    ...
    def get_attribute(self, attribute =ElementAttribute.LABEL, timeout=10, raise_exception=True) -> str:
        ...
        try:
            ...
            if raise_exception:
                raise NotFoundError(f"failed to get attribute {attribute} for element {self.locator} with exception {exception}")
            Element.__logger.info(f"failed to get attribute {attribute} for element {self.locator} with exception {exception}")
    ...
    def scroll_vertically_from_element(self):
        ...
        Element.__logger.info('screen width {} - screen height {} - element_x {} - ''element_y {} '.format(
            screen_width,
            screen_height,
            element_x_position,
            element_y_position
        ...
        Element.__logger.info('horizontal_start_point {} - vertical_start_point {} - horizontal_end_point {} '
        ...
        Element.__driver.swipe(horizontal_start_point, vertical_start_point, horizontal_end_point, vertical_end_point, 500)
        self.element = None
